    ...
    <div className="space-y-6 max-w-[1200px]">
      <div>
        <h1 className="text-2xl font-semibold text-slate-100 mb-1 flex items-center gap-3">
          <SearchIcon className="w-6 h-6 text-indigo-300" />
          Cross-org search
        </h1>
      ...
      {error && <Banner tone="danger" title="Search failed">{error}</Banner>}
      ...
      {!debouncedQ ? (
        ...
        <div className="rounded-2xl border border-slate-800/60 bg-slate-900/40 px-4 py-8 text-center text-slate-500 text-sm">
          Searching…
        </div>
      ) : totalCount === 0 ? (
